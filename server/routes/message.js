...
router.get('', async (request, response) => {
    try {
        // Fetch and group messages by date
...
router.post('', async (request, response) => {
    ...
    try {
        const newMessage = new Message({
            text,
            sender,
            timestamp: new Date()  // Make sure each message has a timestamp
        });
        ...
        if (sender === 'user') {
            ...
            const botMessage = new Message({ text: botResponse, sender: 'bot', timestamp: new Date() });
